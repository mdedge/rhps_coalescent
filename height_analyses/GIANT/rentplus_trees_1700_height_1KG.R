...
height_fn <- "HEIGHT_2014.alleles.for.graham.txt"
...
mark.dat <- fread(height_fn, head = FALSE)
...
save.image(file = "rent_1700_trees_1KG_height_071218.RData")
...
mark.eff <- fread("HEIGHT_2014.gwas", head = TRUE)

inds.eff <- match(mark.dat$V4, mark.eff$CHROM.POS)

beta <- mark.eff$beta[inds.eff]
Z <- mark.eff$Z[inds.eff]
rsID <- mark.eff$rsID[inds.eff]

mark.dat <- cbind(mark.dat, beta, Z, rsID)
...
Qx_test(trajs_neut[time %in% ((0:10)/100),],beta, perms = 10000)


Qx_test(trajs_neut[time %in% ((0:2)/200),], beta, perms = 10000)
Qx_test(trajs_neut[1:100,], beta, perms = 0)

Qx_test(trajs_neut[1:11,], beta, perms = 10000)



save.image(file = "rent_1700_trees_1KG_height_GIANT_processed_071218.RData")


######################################################################################3
#Plot GIANT and UKB on one figure

load("rent_1700_trees_1KG_height_GIANT_processed_071218.RData")
xl <- c(-.2,0)

pdf("height_lr_3ests_GIANTandUKB.pdf", width = 6, height = 8 )
par(cex = 0.85, mfrow = c(2,1))
plot(-time, traj.phen.neut, type = "l", xlab = "Time (coalescent units)", ylab = "Estimated mean polygenic height (UK 1000G)", ylim = c(-2, 2), xlim = xl, xaxt = "n", bty = "n")
axis(1, at = c(0, -.1, -.2))
polygon(c(-time, rev(-time)), c(traj.phen.neut + 1.96*sqrt(var.phen.neut.bin), rev(traj.phen.neut - 1.96*sqrt(var.phen.neut.bin))) , col = pal.int[1], border = NA)
polygon(c(-time, rev(-time)), c(traj.phen.wt_l1 + 1.96*sqrt(var.phen.wt_l1), rev(traj.phen.wt_l1  - 1.96*sqrt(var.phen.wt_l1 ))) , col = pal.int[3], border = NA)
polygon(c(-time[1:1000], rev(-time[1:1000])), c(traj.phen.mom_smoothtime[1:1000] + 1.96*sqrt(var.phen.mom_smoothtime[1:1000]), rev(traj.phen.mom_smoothtime[1:1000]  - 1.96*sqrt(var.phen.mom_smoothtime[1:1000] ))) , col = pal.int[2], border = NA)
...
load("../UKbiobank/rent_1700_trees_1KG_height_UKB_processed_071218.RData")
