...
traj.fn <- "temp_out8/temp.txt"
msout.fn <- "temp_out8/ms_out.txt"
rent_in_fn <- "temp_out8/rent_in.txt"
...
for(k in 1:dim(pars)[1]){
	...
	if((k - 1) %% length(n_chromss) == 0){
		print(k)
		print("simulating polygenic score trajectory")
...
fn_str <- paste("qxmat_ton", as.character(t), "_toff", as.character(t.off), sep = "")
save(list = c("pars", "qxtest_mat"), paste(fn_str, ".RData", sep = ""))
